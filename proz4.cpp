...
void random_wait() {
    int wait_time = rand() % 500; // Losowy czas w milisekundach (do 500 ms)
    ...
    if (msg.tag == TAG_APPROVE) {
        ...
        if (msg.mpc_id == mpc_to_request) {
            ...
            if ((msg.lamport_time < lamport_time) ||
                ...
                // Wyślij TAG_APPROVE do procesu, który ma pierwszeństwo
        ...
        mpc_status[msg.mpc_id] = {msg.mpc_id, -1}; // Aktualizacja tablicy stanów MPC
